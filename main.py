...
async def bypass_unlockify(url):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # 1. Did it auto-redirect to Telegram?
            if "t.me" in page.url:
            ...
            # 2. Check for the data-reward-url tag we found earlier
            reward_match = re.search(r'data-reward-url=["\'](https://t\.me/[^"\']+)["\']', content)
            if reward_match:
                print("✅ SUCCESS! Found Telegram link in data-reward-url attribute.")
                return reward_match.group(1)

            # 3. Fallback standard regex
...
# ====================================================================
# FAST CURL_CFFI SCRAPER FOR FILES.FM / DEEP LINKS
# ====================================================================
def scrape_target_url(url, allowed_domains):
    ...
    try:
        ...
        # Catch data-reward-url just in case the server IP didn't get flagged this time
        ...
        if "unlockify.ink" in intermediary_link:
            return "UNLOCKIFY", intermediary_link
        ...
        if response2.status_code == 403: return None, f"❌ Intermediary page blocked us (403): {intermediary_link}"
...
def get_all_links(event):
    ...
    if event.message.entities:
        for ent in event.message.entities:
            ...
            elif isinstance(ent, MessageEntityUrl): urls.add(event.text[ent.offset : ent.offset + ent.length])
    return list(urls)
...
async def process_single_link(url_to_visit, chat_name):
    ...
    if is_firestore_site:
        ...
        debug_content = "Unlockify Processed via Playwright"
    else:
        loop = asyncio.get_running_loop()
...
async def handler(event):
    ...
    # Process links with a staircase delay to bypass Cloudflare/StackProtect
